...
void PlatformWindowGeneric::SwapBuffers()
{
	...
	PlatformUtils::Get().MoveCursor(m_Framebuffer->Width + m_ViewportStartX, 0);
	std::cout << ' ';
	for (ConsoleUpdate& cupdate : m_Updates)
	{
		if (cupdate.bMoveCursor)
		{
			PlatformUtils::Get().MoveCursor(cupdate.x + m_ViewportStartX, cupdate.y + m_ViewportStartY);
		}
		std::cout << cupdate.text;
	}
...
void PlatformWindowGeneric::FlushBatchRun(int32& lastCursorX, int32 lastBatchX, int32 row)
{
	...
	consoleUpdate.x = m_BatchStart + 1;
	consoleUpdate.y = row + 1;
	consoleUpdate.bMoveCursor = lastCursorX != m_BatchStart - 1;

	lastCursorX = lastBatchX;

	Cell* lastBatchProps = (m_Updates.size() > 0) ? &m_LastBatchProperties : nullptr;
	std::string escapeSequence = BuildEscapeSequence(&m_BatchProperties, lastBatchProps);
	escapeSequence += m_BatchBuffer;


	consoleUpdate.text = escapeSequence;
	m_Updates.push_back(std::move(consoleUpdate));
